...
void calibrate_gyro()
{
	...
	for (int i = 0; i < 100; i++)
	{	
		gyro_request_raw(0, &dataX[i], &dataY[i], &dataZ[i]);
		if (i > 1)
		{ 
			if (abs(dataX[i] - dataX[i-1]) >= 5)
				dataX[i] = dataX[i-1];
			if (abs(dataY[i] - dataY[i-1]) >= 5)
				dataY[i] = dataY[i-1];
			if (abs(dataZ[i] - dataZ[i-1]) >= 5)
				dataZ[i] = dataZ[i-1];
		}
		sumX += dataX[i];
		sumY += dataY[i];
		sumZ += dataZ[i];
		Sleep(1);
		printf("X: %f ; Y: %f ; Z: %f\nDelta X: %f ; Delta Y: %f ; Delta Z: %f \n ********************* \n", dataX[i], dataY[i], dataZ[i], abs(dataX[i] - dataX[i-1]), abs(dataY[i] - dataY[i-1]), abs(dataZ[i] - dataZ[i-1]));
	}
	float medX = sumX / 100.0, medY = sumY / 100.0, medZ = sumZ / 100.0;

	float refs[3] = {0.0};
	recieve_unican_message(0x19, 0x0BE8, (uint8_t*)(refs), 96, 1000);
	float refX = refs[0];
	float refY = refs[1];
	float refZ = refs[2];
	
	gyro_set_offset(0, medX + refX, medY + refY, medZ + refZ);
	printf("Set offset: X: %f ; Y: %f ; Z: %f", medX + refX, medY + refY, medZ + refZ);
)
}
// 
//
